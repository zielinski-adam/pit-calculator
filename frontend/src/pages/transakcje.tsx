...
const columns: ColumnDef<Trade, unknown>[] = [
  ...
  { accessorKey: "asset_category", header: "Kategoria", cell: ({ getValue }) => {
    ...
    return v === "Stocks" ? "Akcje" : v === "Equity and Index Options" ? "Opcje" : v;
  }},
  ...
  { accessorKey: "quantity", header: "Ilość", cell: ({ getValue }) => {
    const q = Number(getValue() as string);
    return <span className={q > 0 ? "text-green-600" : "text-red-600"}>{fmtQty(getValue() as string)}</span>;
  }},
  { accessorKey: "price", header: "Cena", cell: ({ getValue }) => fmtPLN(getValue() as string, 4) },
  { accessorKey: "proceeds", header: "Kwota", cell: ({ getValue }) => fmtPLN(getValue() as string) },
  { accessorKey: "commission", header: "Prowizja", cell: ({ getValue }) => fmtPLN(getValue() as string) },
